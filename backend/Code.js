...
[0. 본문 이미지 삽입 문구 최우선 규칙]
- 본문에서 "📷 [이미지 N 삽입 위치: 구도 요약]" 문구를 먼저 모두 찾아 이미지 번호와 구도 요약을 추출하십시오.
- 이미지 1, 2, 3은 각각 같은 번호의 이미지 삽입 문구를 1:1로 따르십시오. 괄호 안 구도 요약을 해당 이미지의 최우선 시각 지시로 사용하고, 삽입 문구 앞뒤 문단의 장비·동작·산업 환경을 보조 맥락으로 반영하십시오.
- 본문에 명시된 이미지 삽입 문구를 일반적인 교정실, 클린룸, 엔지니어 작업 장면으로 임의 교체하지 마십시오.
- 이미지 삽입 문구가 3개보다 적을 때만 남는 이미지를 본문의 다른 핵심 섹션에서 도출하십시오.
- 각 프롬프트의 첫 부분에는 해당 삽입 문구에서 읽은 구체적인 피사체와 동작을 먼저 명시하십시오.

[1. 이미지 프롬프트 작성 규칙 (Midjourney v6 초고화질 실사 표준)]
- 모든 프롬프트는 영문(English)으로 작성하고 Subject & Action, Environment, Lighting, Camera & Quality를 포함하십시오.
- 장비 종류, 측정 대상, 작업 동작, 배경은 반드시 해당 이미지 삽입 문구와 인접 본문에 근거하십시오. 본문에 없는 장비나 공정을 임의로 추가하지 마십시오.
- 세 프롬프트는 본문이 요구하는 장면에 맞춰 구도, 카메라 거리, 조명, 배경을 서로 다르게 설계하십시오. 매번 같은 클린룸·같은 엔지니어·같은 카메라 렌즈를 반복하지 마십시오.
- 카메라와 렌즈는 장면에 맞게 선택하십시오. 공간 전경은 24mm/35mm, 제품·작업 장면은 50mm/85mm, 정밀 부품은 90mm/100mm macro 등으로 구분할 수 있습니다.
- 인물은 이미지 삽입 문구나 인접 본문에서 필요할 때만 등장시키고, 등장하는 경우 반드시 한국인(Korean engineer, Asian professional)으로 명시하십시오.
- 모든 프롬프트 끝에는 photorealistic, ultra-detailed, realistic industrial photography, no text, no watermark, --ar 16:9 --style raw --v 6.0을 포함하십시오.

[2. 캡션 규칙]
- 캡션은 이미지 아래 한 줄에 들어갈 수 있도록 공백 포함 18~30자의 짧은 한글 문구로 작성하십시오.
- 한 문장 설명이나 홍보 문구 대신 이미지의 핵심 장면만 나타내는 간결한 명사형 문구를 사용하십시오.
- 마침표, 말줄임표, 해시태그, 괄호, "~합니다" 표현을 사용하지 마십시오. 글자 수를 맞추기 위해 문장을 중간에서 끊지 마십시오.
- 예시: "압력계 교정 전 영점 상태 확인", "현장 측정값과 기준값 비교"
...
* 캡션: (본문 이미지 삽입 문구를 반영한 18~30자의 한 줄 한글 캡션)
...
먼저 본문의 "📷 [이미지 N 삽입 위치: 구도 요약]" 문구를 모두 찾아 아직 활용되지 않은 장면을 최우선으로 사용하고, 각 삽입 문구 앞뒤 문단의 장비·동작·환경을 프롬프트에 반영해줘.
일반적인 클린룸, 장비 매크로, 엔지니어 장면을 습관적으로 반복하지 말고 본문에 실제로 적힌 장면과 구도를 따라야 해.
각 프롬프트는 본문에서 아직 시각화되지 않은 디테일이나 완전히 새로운 구도를 제안해야 해.
인물은 본문상 필요한 경우에만 포함하고, 등장할 경우 반드시 한국인(Korean people, Korean engineers, etc.)으로 묘사해줘.
캡션은 이미지 핵심 장면만 표현하는 공백 포함 18~30자의 짧은 한글 명사형 문구로 작성하고, 마침표·말줄임표·해시태그·괄호·"~합니다" 표현은 사용하지 마. 글자 수를 맞추기 위해 문장을 중간에서 끊지 마.
